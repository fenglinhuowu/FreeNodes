...
@dataclass
class CheckConfig:
    enabled: bool = True                 # filter merged outputs after crawl
    timeout: float = 6.0
    concurrency: int = 40
    ...
    batch_size: int = 150
    max_sparkle: int = 300               # max proxies written to sparkle.yaml
    parallel_batches: int = 4            # concurrent Mihomo processes for delay test
    # Download speed + site compatibility (after delay test)
    speed_enabled: bool = True
    min_mbps: float = 0.1                # keep nodes with >= this Mbps
    speed_bytes: int = 200_000           # download size for measurement
    speed_timeout: float = 12.0
    speed_concurrency: int = 8
    # Non-Cloudflare download (CF-through-CF was falsely "fast")
    speed_url: str = "http://cachefly.cachefly.net/200kb.test"
    # Must succeed through proxy (optional; empty = skip YouTube/Google gate)
    compat_urls: list[str] = field(default_factory=list)
